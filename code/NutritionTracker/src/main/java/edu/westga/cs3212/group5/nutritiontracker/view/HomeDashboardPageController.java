...
public class HomeDashboardPageController implements ViewModelAware {
	...
	@FXML
	private void goHome() {
		switchTo("HomeDashboardPage.fxml");
	}

	@FXML
	private void goCreateFood() {
		switchTo("CreateFoodItemTypeSelectionPage.fxml");
	}

	@FXML
	private void goAddFood() {
		switchTo("SearchPage.fxml");
	}

	/**
	 * Sets the ViewModel that data will be pulled from. After calling this method,
	 * the {@code calendarPicker.valueProperty()} will be bound bidirectionally to
	 * the ViewModel's {@code selectedDateProperty}, ensuring that changes in either
	 * the UI or the ViewModel are reflected in both.
	 * 
	 * @param viewModel the HomeDashboardViewModel instance to bind to this
	 *                  controller
	 */
	@Override
