...
public class FoodSearchPanelController {
	...
	@FXML
	private ListView<FoodItem> resultsListView;
	@FXML
	private Label statusLabel;

	@FXML
	private HBox portionSizeRow;
	@FXML
	private Spinner<Double> portionSizeSpinner;
	@FXML
	private Label portionUnitLabel;

	@FXML
	private Label selectionLabel;
	@FXML
	private Button addSelectedFoodButton;

	private Consumer<FoodItem> onFoodSelected = null;
	private Consumer<FoodItem> onAddSelectedFood = null;
	...
	void initialize() {
		...
		this.resultsListView.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) -> {
	...
	/**
	 * Switches the panel into ingredient mode by showing the portion size spinner
	 * row.
	 */
	public void enableIngredientMode() {
		...
		if (this.portionSizeSpinner != null) {
			this.portionSizeSpinner.setValueFactory(new SpinnerValueFactory.DoubleSpinnerValueFactory(1, 9999, 1, 1));
			this.portionSizeSpinner.setEditable(true);
		}
	}

	/**
	 * Switches this panel into standalone page mode by making the selection label
	 * and Add button visible.
	 */
	public void enableStandaloneMode() {
		...
		if (this.addSelectedFoodButton != null) {
			...
			this.addSelectedFoodButton.setOnAction(e -> this.fireAddSelectedFood());
		}
	}
	...
	/**
	 * Replaces the action taken when the Add button is pressed.
	 *
	 * @param handler the new handler; null resets to default behaviour
	 */
	public void setOnAddSelectedFood(Consumer<FoodItem> handler) {
		this.onAddSelectedFood = handler;
	}
	...
	/**
	 * Returns the portion size currently shown in the spinner. Defaults to 1.0 if
	 * ingredient mode is not enabled.
	 *
	...
	private void fireAddSelectedFood() {
		if (this.onAddSelectedFood != null) {
			...
			new Alert(Alert.AlertType.INFORMATION, "\"" + this.currentlySelectedFood.getDescription() + "\" added!")
					.showAndWait();
			this.reset();
		}
	}
	...
	private void refreshResults() {
		...
		try {
			...
			this.statusLabel.setText(results.isEmpty() ? "No foods found matching \"" + query + "\"."
					: results.size() + " result(s) — click to select, then press the Add button.");

		} catch (Exception e) {
			this.statusLabel.setText("Error connecting to server.");
	...
	private static class FoodCell extends ListCell<FoodItem> {
		...
		protected void updateItem(FoodItem item, boolean empty) {
			...
			catLabel.setStyle("-fx-background-color: #b8d5a3; -fx-padding: 2 6 2 6; "
