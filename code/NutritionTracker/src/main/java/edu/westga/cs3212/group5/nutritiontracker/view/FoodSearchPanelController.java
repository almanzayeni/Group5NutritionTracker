...
public class FoodSearchPanelController {
	 ...
	 * row. Call this from CreateCompositeFoodPageController after load. The spinner
	 * resets to 1 whenever a new food is selected.
	 */
	...
	private void handleAddSelectedFood(ActionEvent event) {
		...
		new Alert(Alert.AlertType.INFORMATION, "\"" + this.currentlySelectedFood.getDescription()
				+ "\" added! (Persistence hook — wire up in next sprint.)").showAndWait();
	...
	private void refreshResults() {
		...
		if (results.isEmpty()) {
			...
			this.statusLabel.setText(results.size() + " result(s) — click to select, then press the Add button.");
		}
	}
